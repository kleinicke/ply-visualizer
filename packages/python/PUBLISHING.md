...
The current release version is `0.4.0`. Publish releases only when requested;
...
uvx twine check --strict packages/python/dist/ply_visualizer-0.4.0*
```
...
```sh
uvx twine upload --non-interactive --config-file .local/.pypirc --repository pypi packages/python/dist/ply_visualizer-0.4.0-py3-none-any.whl packages/python/dist/ply_visualizer-0.4.0.tar.gz
```
...
```sh
uvx --from ply-visualizer==0.4.0 ply-viewer --help
```
