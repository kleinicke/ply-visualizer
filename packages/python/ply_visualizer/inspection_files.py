...
def export_subset(manager, scene_id, path, name=None):
    file = artifact_path(manager, path, '.ply')
    bridge = manager.get(scene_id)._bridge
    meta = bridge.request('export_subset', {'action': 'start', 'name': name})
...
def scene_state_file(manager, scene_id, action, name, path):
