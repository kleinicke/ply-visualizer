...
describe('NPY File Loading UI Tests', function () {
  ...
  it('should open NPY file with 3D Visualizer', async function () {
    const sideBar = new SideBarView();
    const explorer = await sideBar.getContent().getSection('Explorer');
  ...
  it('should show depth conversion interface for NPY file', async function () {
    ...
    try {
      ...
      const conversionElements = ['Camera Model', 'Focal Length', 'Depth Type'];
      ...
      console.log(
        'Note: Could not verify webview content, this may be expected during initial loading'
      );
