...
describe('XYZ Variants (XYZN, XYZRGB) File Loading UI Tests', function() {
    ...
    xyzVariants.forEach(variant => {
        it(`should open ${variant.name} (${variant.ext}) file with PLY Visualizer`, async function() {
            const sideBar = new SideBarView();
            const explorer = await sideBar.getContent().getSection('Explorer');
            ...
            if (!visualizerOption) {
                throw new Error(`PLY Visualizer option not found in context menu for ${variant.ext} files`);
            }
