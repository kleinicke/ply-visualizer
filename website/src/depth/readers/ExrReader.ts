...
export class ExrReader implements DepthReader {
  ...
  async read(buf: ArrayBuffer): Promise<DepthReaderResult> {
      ...
      if (error instanceof Error && error.message.includes('unsupported data channels')) {
        ...
        try {
          ...
          if (!basicResult.depthData || !basicResult.depthChannelName) {
            ...
            throw new Error(
              `No depth channel found in EXR file. Available channels: ${availableChannels}. Expected: Depth.V, Depth, Z, Depth.Z, Disparity, Disp, or R.`
            );
          }

          console.log(
            `📋 EXR Reader: Basic parser successfully extracted depth from channel: ${basicResult.depthChannelName}`
          );
          ...
          throw new Error(
            `Failed to parse EXR file with both Three.js EXRLoader and basic parser. Three.js error: ${error.message}. Basic parser error: ${basicError instanceof Error ? basicError.message : String(basicError)}`
          );
        }
      }

      throw new Error(
        `Failed to parse EXR file: ${error instanceof Error ? error.message : String(error)}`
      );
    }
    ...
    console.log(
      `📋 EXR Reader: Data type: ${texData.type}, Format: ${texData.format}, Data length: ${texData.data?.length || 0}`
    );
  ...
  private processDepthData(data: {
    ...
    console.log(
      `📋 EXR Reader: Depth data length: ${depthData.length}, expected: ${width * height}`
    );
    ...
    console.log(
      `📋 EXR Reader: Depth analysis - Valid: ${validDepthCount}, Zero: ${zeroCount}, NaN: ${nanCount}, Inf: ${infCount}, Total: ${totalPixels}`
    );
