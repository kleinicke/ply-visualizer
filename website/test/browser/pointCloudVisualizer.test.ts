...
suite('Point Cloud Visualizer Test Suite', () => {
  ...
  test('Should create correct quaternion matrices', () => {
    ...
    const quatZ90 = PointCloudVisualizerTestUtils.createQuaternionMatrix(
      0,
      0,
      Math.sin(Math.PI / 4),
      Math.cos(Math.PI / 4)
  ...
  test('Should create correct angle-axis matrices', () => {
    ...
    const normalizedMatrix = PointCloudVisualizerTestUtils.createAngleAxisMatrix(
      unnormalizedAxis,
      45
    );
  ...
  test('Should validate camera parameters correctly', () => {
    // Valid parameters
    assert.ok(
      PointCloudVisualizerTestUtils.validateCameraParams({ fx: 525, fy: 525, cx: 320, cy: 240 })
    );

    // Invalid parameters
    assert.ok(
      !PointCloudVisualizerTestUtils.validateCameraParams({ fx: 0, fy: 525, cx: 320, cy: 240 })
    );
    assert.ok(
      !PointCloudVisualizerTestUtils.validateCameraParams({ fx: 525, fy: -525, cx: 320, cy: 240 })
    );
