...
`;
  ...
  test('Should test coordinate system and transformations', async function () {
    ...
    console.log(
      `Bounds: min(${bounds.minX},${bounds.minY},${bounds.minZ}) max(${bounds.maxX},${bounds.maxY},${bounds.maxZ})`
    );
    ...
    assert.strictEqual(
      firstNormalized.x,
      -0.5,
      'Origin should be normalized to (-0.5, -0.5, -0.5)'
    );
    assert.strictEqual(
      firstNormalized.y,
      -0.5,
...
`;
  ...
  test('Should verify file size limits and performance thresholds', function () {
    ...
    for (const fileSize of fileSizes) {
      ...
      console.log(
        `${fileSize.name} (~${Math.round(estimatedVertices)} vertices) -> optimize: ${shouldUseOptimization}`
      );

      assert.strictEqual(
        shouldUseOptimization,
        fileSize.shouldOptimize,
