    ...
    try {
      ...
      if (
        ...
        for (let i = 0; i < data.instance_info.length; i++) {
          ...
          const parsedPose = pose.normalizePose(single);
          const group = pose.buildPoseGroup(this, parsedPose);
          this.scene.add(group);
          this.poseGroups.push(group);
          this.poseJoints.push(parsedPose.joints as any);
          this.poseEdges.push(parsedPose.edges);
        ...
        const parsedPose = pose.normalizePose(data);
        const group = pose.buildPoseGroup(this, parsedPose);
        this.scene.add(group);
