...
export class OffParser {
  async parse(data: Uint8Array, timingCallback?: (message: string) => void): Promise<OffData> {
    ...
    for (let i = 0; i < vertexCount && lineIndex < lines.length; i++) {
      ...
      if (parts.length < 3) {
        throw new Error(
          `OFF file: invalid vertex at line ${lineIndex + 1} - expected at least 3 coordinates, got ${parts.length}`
        );
      }
    ...
    for (let i = 0; i < faceCount && lineIndex < lines.length; i++) {
      ...
      if (parts.length < vertexCountForFace + 1) {
        throw new Error(
          `OFF file: face at line ${lineIndex + 1} expects ${vertexCountForFace} vertices but only has ${parts.length - 1}`
        );
      }
      ...
      for (const index of faceIndices) {
        if (index < 0 || index >= vertexCount) {
          throw new Error(
            `OFF file: invalid vertex index ${index} in face at line ${lineIndex + 1}`
          );
        }
      }
    ...
    timingCallback?.(
      `✅ OFF: Parsing complete - ${vertices.length} vertices, ${faces.length} triangles in ${totalTime.toFixed(1)}ms`
    );
