...
export class StlParser {
  ...
  async parse(data: Uint8Array, timingCallback?: (message: string) => void): Promise<StlData> {
    ...
    if (
      headerText.toLowerCase().includes('solid ') &&
    ...
    log(
      `✅ Parser: STL parsing complete in ${(parseTime - parseStartTime).toFixed(1)}ms - ${result.triangleCount} triangles`
    );
  ...
  private parseAsciiTriangle(lines: string[], startIndex: number): StlTriangle | null {
    try {
      ...
      if (
        !lines[startIndex + 1] ||
        !lines[startIndex + 1].trim().toLowerCase().includes('outer loop')
      ) {
        return null;
      }
