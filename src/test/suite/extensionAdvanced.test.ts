...
suite('Extension Advanced Test Suite', () => {
    ...
    test('Should have correct activation events', () => {
        ...
        if (packageJSON?.activationEvents) {
            ...
            assert.ok(activationEvents.includes('onCustomEditor:plyViewer.plyEditor') ||
    ...
    test('Should register custom editor provider', () => {
        ...
        assert.ok(editor.priority === 'default' || editor.priority === undefined, 'Priority should be default or undefined');
    });
    ...
    test('Should register context menu items', () => {
        ...
        assert.ok(commands.includes('plyViewer.openFile'), 'Open file command should be in context menu');
        assert.ok(commands.includes('plyViewer.convertTifToPointCloud'), 'TIF conversion should be in context menu');
    });
    ...
    test('Should support extension dependencies', () => {
        ...
        if (extensionDependencies) {
            ...
            for (const dep of extensionDependencies) {
                assert.ok(typeof dep === 'string' && dep.includes('.'), 'Dependency should be valid extension ID');
            }
        }
    });
    ...
    test('Should handle workspace trust requirements', () => {
        ...
        if (capabilities?.untrustedWorkspaces) {
            ...
            assert.ok(typeof untrustedWorkspaces.supported === 'boolean', 
                     'Untrusted workspace support should be boolean');
            
            if (untrustedWorkspaces.description) {
                assert.ok(typeof untrustedWorkspaces.description === 'string',
