...
import { PngReader, Enhanced16BitPngReader, PngDepthConfig } from '../../webview/depth/readers/PngReader';
...
suite('PngReader', () => {
    ...
    suite('detectPngFormat', () => {
        function createMockPngHeader(width: number, height: number, bitDepth: number, colorType: number): Uint8Array {
    ...
    suite('read', () => {
        function createBasicPngBuffer(): ArrayBuffer {
            ...
            const ihdrLength = 13;
            const ihdrType = [73, 72, 68, 82]; // "IHDR"
            const width = 2, height = 2, bitDepth = 8, colorType = 0;
            ...
            // IHDR chunk
            data[offset++] = 0; data[offset++] = 0; data[offset++] = 0; data[offset++] = ihdrLength;
            for (let i = 0; i < ihdrType.length; i++) {
                data[offset++] = ihdrType[i];
            }
            
            // IHDR data
            data[offset++] = 0; data[offset++] = 0; data[offset++] = 0; data[offset++] = width;
            data[offset++] = 0; data[offset++] = 0; data[offset++] = 0; data[offset++] = height;
            data[offset++] = bitDepth;
            data[offset++] = colorType;
            data[offset++] = 0; data[offset++] = 0; data[offset++] = 0;
            
            // CRC
            data[offset++] = 0; data[offset++] = 0; data[offset++] = 0; data[offset++] = 0;
    ...
    suite('read', () => {
        test('should throw not implemented error', async () => {
            ...
            try {
                ...
                assert.ok(error.message.includes('Enhanced 16-bit PNG reader requires pngjs library integration'));
            }
        });
    });
