...
class MainGeometryMathUtils {
  ...
  static calculateRayIntersection(
    mouse: THREE.Vector2,
  ...
  static processVerticesForGeometry(
    ...
    const hasColors = vertices.some(
      v => v.red !== undefined || v.green !== undefined || v.blue !== undefined
    );
    if (hasColors) {
  ...
  static calculateVertexNormals(
    vertices: Array<{ x: number; y: number; z: number; nx?: number; ny?: number; nz?: number }>
  ): {
    hasNormals: boolean;
    normals: Float32Array;
  } {
    const hasNormals = vertices.some(
      v => v.nx !== undefined || v.ny !== undefined || v.nz !== undefined
    );
  ...
  static convertDepthToVertices(
    depthData: Float32Array,
    width: number,
    height: number,
    cameraParams: { fx: number; fy: number; cx: number; cy: number }
  ...
  static calculateAdaptivePointSize(
    baseSize: number,
    distance: number,
    minSize: number = 0.5,
    maxSize: number = 10
...
suite('Main Geometry and Math Functions Test Suite', () => {
  test('Should parse space-separated values correctly', () => {
    ...
    assert.deepStrictEqual(
      MainGeometryMathUtils.parseSpaceSeparatedValues('1.5  2.7   3.14'),
      [1.5, 2.7, 3.14]
    );
    assert.deepStrictEqual(
      MainGeometryMathUtils.parseSpaceSeparatedValues('  -1   0  1  '),
      [-1, 0, 1]
    );
  ...
  test('Should calculate optimal camera distance', () => {
    ...
    const largeDistance = MainGeometryMathUtils.calculateOptimalCameraDistance(
      largeBoundingBox,
      fov
    );
  ...
  test('Should convert depth data to vertices', () => {
    ...
    const vertices = MainGeometryMathUtils.convertDepthToVertices(
      depthData,
      width,
      height,
      cameraParams
