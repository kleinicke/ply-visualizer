...
suite('NPY Reader Test Suite', function () {
  ...
  test('should read test NPY files correctly', async function () {
    ...
    for (const testFile of testFiles) {
      ...
      try {
        ...
        console.log(
          `✅ Successfully read ${testFile.name}: ${result.image.width}x${result.image.height}, range: ${minValue.toFixed(3)}-${maxValue.toFixed(3)}`
        );
      } catch (error) {
        assert.fail(
          `Failed to read ${testFile.name}: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  });
  ...
  test('should handle invalid NPY files gracefully', async function () {
    ...
    try {
      ...
      assert.ok(
        errorMessage.includes('Invalid NPY file'),
