...
`;
    ...
    test('Should parse real facemesh wireframe OBJ+MTL files', async () => {
        const objFilePath = path.join(__dirname, '../../../testfiles/new_formats/facemesh_wireframe 1.obj');
        const mtlFilePath = path.join(__dirname, '../../../testfiles/new_formats/facemesh_wireframe 1.mtl');
        ...
        if (fs.existsSync(objFilePath) && fs.existsSync(mtlFilePath)) {
            ...
            console.log(`Facemesh integration test: ${objResult.vertexCount} vertices, ${objResult.lineCount} lines, ${mtlResult.materials.size} material(s)`);
        } else {
...
`;
        ...
        const hexColor = (Math.round(rgb.r * 255) << 16) | 
