...
suite('PLY Viewer Extension Test Suite', () => {
  ...
  test('Multiple PLY commands should be registered', async () => {
    ...
    assert.ok(
      hasOpenFile || hasMultipleFiles || hasTifConvert,
      'At least one PLY command should be registered'
    );
  });
...
`;
    ...
    if (fs.existsSync(testTifPath)) {
      try {
        ...
        assert.ok(
          ...
          'Expected error type for TIF conversion without parameters'
        );
      }
    }
  });
  ...
  test('Should have correct context menu contributions', () => {
    ...
    if (ext) {
      ...
      const openFileItem = contextMenuItems.find(
        (item: any) => item.command === 'plyViewer.openFile'
      );
      const convertTifItem = contextMenuItems.find(
        (item: any) => item.command === 'plyViewer.convertTifToPointCloud'
      );
      const loadJsonPoseItem = contextMenuItems.find(
        (item: any) => item.command === 'plyViewer.loadJsonAsPose'
      );
  ...
  test('Extension should have correct metadata', () => {
    ...
    if (ext) {
      ...
      assert.strictEqual(packageJSON.displayName, '3D Point Cloud and Mesh Visualizer (PLY, ...)');
      assert.strictEqual(packageJSON.publisher, 'kleinicke');
      assert.ok(packageJSON.version, 'Version should be defined');
      assert.ok(
        packageJSON.description.includes('3D visualizer'),
        'Description should mention 3D visualization'
      );
