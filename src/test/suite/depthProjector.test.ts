...
suite('Depth Projector Test Suite', () => {
    ...
    class TestDepthProjector {
        ...
        validateCameraParameters(params: any): boolean {
            return params && 
                   typeof params.fx === 'number' && params.fx > 0 &&
                   typeof params.fy === 'number' && params.fy > 0 &&
    ...
    test('Should project depth image to 3D point cloud with pinhole camera', () => {
        ...
        const depthData = new Float32Array([
            1.0, 2.0,  // First row
            3.0, 0.0   // Second row (last pixel has 0 depth = invalid)
        ]);
    ...
    test('Should handle invalid depth values', () => {
        ...
        const height = 1;
        const depthData = new Float32Array([
            -1.0, 0.0, NaN  // All invalid depths
        ]);
    ...
    test('Should handle different depth units and scales', () => {
        ...
        const resultMM = projector.projectToPointCloud(depthData, width, height, cameraParams, metadataMM);
        ...
        const resultM = projector.projectToPointCloud(depthData, width, height, cameraParams, metadataM);
    ...
    test('Should handle disparity conversion', () => {
        ...
        const result = projector.projectToPointCloud(disparityData, width, height, cameraParams, metadata);
    ...
    test('Should handle coordinate system transformations', () => {
        ...
        const resultCV = projector.projectToPointCloud(depthData, width, height, cameraParamsCV, metadataCV);
        ...
        const resultGL = projector.projectToPointCloud(depthData, width, height, cameraParamsGL, metadataGL);
