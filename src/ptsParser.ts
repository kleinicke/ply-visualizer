...
export interface PtsData {
    vertices: Array<{ x: number; y: number; z: number; red?: number; green?: number; blue?: number; nx?: number; ny?: number; nz?: number; intensity?: number }>;
    vertexCount: number;
    hasColors: boolean;
    hasNormals: boolean;
...
export class PtsParser {
    async parse(data: Uint8Array, timingCallback?: (message: string) => void): Promise<PtsData> {
        ...
        const vertices: Array<{ x: number; y: number; z: number; red?: number; green?: number; blue?: number; nx?: number; ny?: number; nz?: number; intensity?: number }> = [];
        ...
        timingCallback?.(`✅ PTS: Parsing complete - ${vertices.length} points in ${totalTime.toFixed(1)}ms`);
