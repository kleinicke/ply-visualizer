...
export class PcdParser {
  async parse(data: Uint8Array, timingCallback?: (message: string) => void): Promise<PcdData> {
    ...
    const hasNormals = fields.some(f =>
      ['normal_x', 'normal_y', 'normal_z', 'nx', 'ny', 'nz'].includes(f.toLowerCase())
    );

    timingCallback?.(
      `📊 PCD: Header parsed - ${vertexCount} points, ${format} format, fields: ${fields.join(', ')}`
    );
    ...
    if (format === 'ascii') {
      ...
      if (binaryData.length < pointSize * vertexCount) {
        throw new Error(
          `PCD binary data too short: expected ${pointSize * vertexCount} bytes, got ${binaryData.length}`
        );
      }
