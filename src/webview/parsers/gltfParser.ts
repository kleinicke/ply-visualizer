...
export class GltfParser {
    async parse(data: Uint8Array, timingCallback?: (message: string) => void): Promise<GltfData> {
        ...
        if (data.length >= 12 && header.getUint32(0, true) === 0x46546C67) { // 'glTF' magic
            // GLB format
            format = 'glb';
            ...
            const jsonChunkLength = new DataView(data.buffer, data.byteOffset + offset, 4).getUint32(0, true);
            const jsonChunkType = new DataView(data.buffer, data.byteOffset + offset + 4, 4).getUint32(0, true);
            offset += 8;
            ...
            if (offset + 8 <= data.length) {
                const binChunkLength = new DataView(data.buffer, data.byteOffset + offset, 4).getUint32(0, true);
                const binChunkType = new DataView(data.buffer, data.byteOffset + offset + 4, 4).getUint32(0, true);
                offset += 8;
        ...
        timingCallback?.(`✅ GLTF: Parsing complete - ${vertices.length} vertices, ${faces.length} triangles in ${totalTime.toFixed(1)}ms`);
    ...
    private getAccessorData(gltfJson: any, accessorIndex: number, binaryData: Uint8Array | null, timingCallback?: (message: string) => void): Float32Array | Uint16Array | Uint32Array | null {
        ...
        if (!gltfJson.buffers || !gltfJson.buffers[bufferView.buffer]) {
            console.warn(`GLTF: Buffer ${bufferView.buffer} not found - this GLTF file is missing buffer data`);
            return null;
        }
        ...
        switch (type) {
            case 'SCALAR': componentsPerElement = 1; break;
            case 'VEC2': componentsPerElement = 2; break;
            case 'VEC3': componentsPerElement = 3; break;
            case 'VEC4': componentsPerElement = 4; break;
            case 'MAT2': componentsPerElement = 4; break;
            case 'MAT3': componentsPerElement = 9; break;
            case 'MAT4': componentsPerElement = 16; break;
        }
