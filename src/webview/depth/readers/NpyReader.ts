 ...
 */
function parseNpzFile(arrayBuffer: ArrayBuffer): { [key: string]: { data: Float32Array, shape: number[], dtype: string } } {
...
export class NpyReader implements DepthReader {
    ...
    async read(arrayBuffer: ArrayBuffer): Promise<DepthReaderResult> {
        ...
        if (arrayBuffer.byteLength >= 4) {
            ...
            if (zipMagic === 0x04034b50) { // ZIP file signature
        ...
        if (shape.length === 3) {
            channels = shape[2];
            if (channels > 4) {
                throw new Error(`Too many channels: ${channels}. Expected format: 3D array with shape (height, width, channels) where channels ≤ 4.`);
            }
        }
        ...
        if (dtype === '<f4' || dtype === '=f4' || dtype === '>f4') {
            ...
            if (availableBytes < expectedBytes) {
                throw new Error(`Insufficient data: expected ${expectedBytes} bytes, got ${availableBytes}`);
            }
        ...
        } else if (dtype.startsWith('<i') || dtype.startsWith('=i') || dtype.startsWith('>i') ||
                   dtype.startsWith('<u') || dtype.startsWith('=u') || dtype.startsWith('>u')) {
            ...
            for (let i = 0; i < expectedElements; i++) {
                ...
                if (bytesPerElement === 1) {
                    ...
                    value = isUnsigned ? dataView.getUint16(offset, littleEndian) : dataView.getInt16(offset, littleEndian);
                } else if (bytesPerElement === 4) {
                    value = isUnsigned ? dataView.getUint32(offset, littleEndian) : dataView.getInt32(offset, littleEndian);
                } else {
            ...
            throw new Error(`Unsupported dtype: ${dtype}. Supported types: float32, float64, int8, int16, int32, int64, uint8, uint16, uint32, uint64.`);
        }
    ...
    private async handleNpzFile(arrayBuffer: ArrayBuffer): Promise<DepthReaderResult> {
        ...
        if (arrayNames.length === 0) {
            throw new Error('NPZ file contains no readable arrays. Expected format: NPZ archive containing at least one 2D NumPy array with depth data (float32/float64 preferred).');
        }
        ...
        if (!selectedData || !selectedArray) {
            ...
            throw new Error(`NPZ file contains no suitable 2D arrays for depth data.\n\nAvailable arrays:\n${arrayInfo}\n\nExpected format: 2D NumPy array with shape (height, width) containing depth/disparity values.`);
        }
        
        // Validate that it's 2D
        if (selectedData.shape.length !== 2) {
            throw new Error(`Selected array "${selectedArray}" has ${selectedData.shape.length}D shape [${selectedData.shape.join(', ')}]. Expected format: 2D array with shape (height, width) for depth data.`);
        }
