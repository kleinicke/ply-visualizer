...
export class PfmReader implements DepthReader {
  ...
  async read(arrayBuffer: ArrayBuffer): Promise<DepthReaderResult> {
    ...
    let offset = 0;
    // Header
    let r1 = readAsciiLine(view, offset); offset = r1.next; const id = r1.line.trim();
    if (id !== 'Pf' && id !== 'PF') throw new Error('Invalid PFM header');
    const channels = id === 'PF' ? 3 : 1;
    let r2 = readAsciiLine(view, offset); offset = r2.next; const dims = r2.line.trim().split(/\s+/);
    if (dims.length < 2) throw new Error('Invalid PFM dimensions');
    const width = parseInt(dims[0], 10);
    const height = parseInt(dims[1], 10);
    let r3 = readAsciiLine(view, offset); offset = r3.next; const scaleStr = r3.line.trim();
